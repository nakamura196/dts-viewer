  ...
  if (isUrl(formatted)) {
    return (
      <a 
        ...
        className="text-[var(--ds-primary)] hover:underline break-all"
      >
        {formatted}
      </a>
    );
...
export default function DublinCoreMetadata({ dublinCore, className = '' }: DublinCoreMetadataProps) {
  ...
  return (
    <div className={`bg-[var(--ds-surface-2)] rounded-lg p-4 ${className}`}>
      <h3 className="text-lg font-semibold text-[var(--ds-fg)] mb-3">
        {t('metadata')}
      </h3>
        ...
        {fields.map(field => {
          ...
          return (
            <div key={field.key} className="flex flex-col sm:flex-row sm:gap-4">
              <dt className="text-sm font-medium text-[var(--ds-fg-muted)] sm:w-24 flex-shrink-0">
                {field.label}:
              </dt>
              <dd className="text-sm text-[var(--ds-fg)] mt-1 sm:mt-0">
                {renderValue(value)}
              </dd>
            </div>
          );
